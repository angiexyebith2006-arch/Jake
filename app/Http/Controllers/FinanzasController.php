...
class FinanzasController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $validated['monto'] = round($validated['monto'], 2);
    ...
    public function reporteCsv(Request $request)
    {
        ...
        fputcsv($handle, ['Total Egresos:',  '$' . number_format($totalEgresos,  2, ',', '.'), '', '', '', '', '']);
        fputcsv($handle, ['Balance General:', '$' . number_format($balance,       2, ',', '.'), '', '', '', '', '']);
    ...
    private function generarHtmlReporte($movimientos, $totalIngresos, $totalEgresos, $balance)
    {
        ...
        <head>
            ...
            <style>
                ...
                .stat-box { background: #F8F9FA; padding: 10px; border-radius: 5px; width: 25%; text-align: center; border: 1px solid #E1E8ED; }
                ...
                .footer { text-align: center; margin-top: 20px; padding-top: 10px; border-top: 1px solid #BDC3C7; font-size: 10px; color: #7F8C8D; }
                button { background: #3498DB; color: white; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer; font-size: 14px; margin-bottom: 20px; }
        ...
        <body>
            ...
            <div class="stats">
                <div class="stat-box">
                    <h3 class="ingreso">$' . number_format($totalIngresos, 2, ',', '.') . '</h3>
                    <p>📈 Total Ingresos</p>
                </div>
                <div class="stat-box">
                    <h3 class="egreso">$' . number_format($totalEgresos, 2, ',', '.') . '</h3>
                    <p>📉 Total Egresos</p>
                </div>
                <div class="stat-box">
                    <h3 class="' . ($balance >= 0 ? 'balance-positivo' : 'balance-negativo') . '">$' . number_format($balance, 2, ',', '.') . '</h3>
            ...
            <table>
                <thead>
                    <tr>
                        <th>ID</th><th>Fecha</th><th>Categoría</th><th>Tipo</th><th>Monto</th><th>Descripción</th>
                    </tr>
                </thead>
                <tbody>';
        ...
        foreach ($movimientos as $movimiento) {
            ...
            $html .= '
                    <tr>
                        ...
                        <td>' . $tipo . '</td>
                        <td class="' . $claseMonto . '">$' . number_format($movimiento->monto, 2, ',', '.') . '</td>
